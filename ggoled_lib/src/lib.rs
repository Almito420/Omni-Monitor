...
// Nova Pro Omni GameDAC (PID 0x2290):
//   report ID 0x01, 7-byte header, 1028-byte bitmap (column-major, identical
//   encoding to NovaPro; first 64 of 128 encoded columns are physically visible)
const OMNI_BITMAP_SIZE: usize = 1028;
const OMNI_REPORT_SIZE: usize = 1 + 7 + OMNI_BITMAP_SIZE; // = 1036

/// Which protocol variant to use for a connected device.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
enum DeviceModel {
    /// Arctis Nova Pro (Wired / Wireless / Xbox) — report ID 0x06, 5-byte header
    NovaPro,
    /// Nova Pro Omni GameDAC (PID 0x2290) — report ID 0x01, 7-byte header
    NovaProOmni,
}
...
impl Device {
    ...
    pub fn connect() -> anyhow::Result<Device> {
        ...
        // ── Nova Pro Omni GameDAC (PID 0x2290) ──────────────────────────────
        // Single HID interface, usage_page 0xFFC0.  Always DeviceMerge::Merged.
        if let Some(info) = api.device_list().find(|d| {
            ...
            let dev = info
                .open_device(&api)
                .map_err(|_| anyhow::anyhow!("Failed to open Nova Pro Omni GameDAC"))?;
            return Ok(Device {
                dev:    DeviceMerge::Merged(dev),
                model:  DeviceModel::NovaProOmni,
                width:  SCREEN_WIDTH,
                height: SCREEN_HEIGHT,
            });
        }

        // ── Existing Arctis Nova Pro devices ────────────────────────────────
    ...
    pub fn dump_devices() {
        ...
        for info in device_infos {
            ...
            println!("usage_page={:#06x}", info.usage_page());
    ...
    // Creates a HID feature report for a `ReportDrawable`.
    fn create_report(&self, d: &ReportDrawable) -> Vec<u8> {
        match self.model {
            DeviceModel::NovaPro => {
            ...
            DeviceModel::NovaProOmni => {
                // Omni report layout (1036 bytes):
                //   [0]       0x01         report ID
                //   [1]       0x93         OLED write command
                //   [2]       dst_x        0x00 = left half, 0x40 = right half
                //   [3]       0x00         row start
                //   [4]       0x40         64 columns per report
                //   [5]       0x40         64 rows
                //   [6..7]    0x00 0x00    exclusive-mode mask (suppresses firmware UI)
                //   [8..1036] bitmap       column-major, 1 bpp, identical encoding to NovaPro
                //
                // Physical display: first 64 of the 128 encoded columns are visible.
                // The split produced by prepare_for_report (SCREEN_REPORT_SPLIT_SZ=64)
                // places exactly 64 columns per drawable, so those 64 land in the
                // visible region and the remaining 512 bitmap bytes stay zero.
                let mut report = vec![0u8; OMNI_REPORT_SIZE];
                report[0] = 0x01;
                report[1] = 0x93;
                report[2] = d.dst_x as u8; // 0x00 (left) or 0x40 (right)
                report[3] = 0x00;
                report[4] = 0x40;
                report[5] = 0x40;
                // report[6..7] = 0x00 0x00  (already zero)

                // Height is always SCREEN_HEIGHT (64) which is already a multiple of 8.
                let padded_h = SCREEN_HEIGHT;
                for y in 0..d.h {
                    for x in 0..d.w {
                        let ri = x * padded_h + y;
                        let pi = (d.src_y + y) * d.bitmap.w + (d.src_x + x);
                        // Bitmap starts at byte offset 8 (2 extra mask bytes vs NovaPro's 6)
                        report[(ri / 8) + 8] |= (d.bitmap.data[pi] as u8) << (ri % 8);
                    }
                }
                report
            }
    ...
    pub fn set_brightness(&self, value: u8) -> anyhow::Result<()> {
        ...
        match self.model {
            ...
            DeviceModel::NovaProOmni => {
                // Command 0x85 assumed; unverified on hardware.
                // Capture USB traffic while SteelSeries GG changes brightness
                // to confirm the exact command bytes if this has no effect.
    ...
    pub fn probe(&self) -> anyhow::Result<()> {
        match self.model {
            ...
            DeviceModel::NovaProOmni => {
                // The Omni exposes battery info via a separate HID usage page
                // (BATT_USAGE_PAGE 0xFF00); probing via the OLED interface is
                // not yet reverse-engineered.  No-op for now.
            }
        }
        Ok(())
    }
    ...
    pub fn return_to_ui(&self) -> anyhow::Result<()> {
        match self.model {
            ...
            DeviceModel::NovaProOmni => {
                // Command 0x95 causes a visible flash on the Omni and should
                // not be used in a render loop.  No-op until the correct
                // return-to-UI sequence is reverse-engineered.
            }
        }
        Ok(())
    }
